		...
		<td valign="top">
			<table class="form-table">
				...
				<tbody>
					...
					<tr>
						<th>Time</th>
						<td><?php echo (!empty($record->schedule_time) ? date($this->config->item('server_display_time_format'), strtotime($record->schedule_time)) : '');?></td>
					</tr>
					<tr>
						<th>Phase</th>
						<td><?php echo (!empty($record) ? $record->schedule_phase : '');?></td>
					</tr>
					<tr>
						<th>Location</th>
						<td><?php echo (!empty($record) ? $record->m_grid_text : '');?></td>
					</tr>
				</tbody>
			</table>
		</td>
...
function material_inventory_picklist_detail_full_list_load_table(table_id){
	jQuery('#' + table_id).jqGrid({
		...
		url: "<?php echo site_url('material/inventory_picklist_pakan/get_list_detail_json');?>", 
		postData : {
