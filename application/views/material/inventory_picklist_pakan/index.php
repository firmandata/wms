...
if (is_authorized('material/inventory_picklist_pakan', 'update')){?>
...
if (is_authorized('material/inventory_picklist_pakan', 'update')){?>
	<button id="material_inventory_picklist_pickingshipment_btn" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">
		...
		<span class="ui-button-text">Ambil dan Tebar</span>
	</button>
<?php
}
if (is_authorized('material/inventory_picklist_pakan', 'delete')){?>
...
<?php 
echo form_open('material/inventory_picklist_pakan/index',
	array(
...
jQuery(document).ready(function(){
	...
	jQuery("#material_inventory_picklist_new_btn").click(function(){
		jquery_dialog_form_open('material_inventory_picklist_form_container', "<?php echo site_url('material/inventory_picklist_pakan/form');?>", {
			form_action : "material/inventory_picklist_pakan/insert"
		}, 
	...
	jQuery("#material_inventory_picklist_edit_btn").click(function(){
		...
		if (id)
		{
			...
			jquery_dialog_form_open('material_inventory_picklist_form_container', "<?php echo site_url('material/inventory_picklist_pakan/form');?>", {
				form_action : "material/inventory_picklist_pakan/update",
				id : row_data.id
			}, 
			function(form_dialog){
				material_inventory_picklist_form_submit(function(data, textStatus, jqXHR){
	...
	jQuery("#material_inventory_picklist_detail_btn").click(function(){
		...
		if (id)
		{
			...
			jquery_dialog_form_open('material_inventory_picklist_form_detail_container', "<?php echo site_url('material/inventory_picklist_pakan/form_detail');?>", {
				id : row_data.id
			}, 
			null,
			{
	...
	jQuery("#material_inventory_picklist_pickingshipment_btn").click(function(){
		...
		if (id)
		{
			...
			var _pickingshipment_url = "<?php echo site_url('material/inventory_picklist_pakan/picking_shipment/@id');?>";
	...
	jQuery("#material_inventory_picklist_delete_btn").click(function(){
		...
		if (id)
		{
			...
			var _delete_url = "<?php echo site_url('material/inventory_picklist_pakan/delete/@id');?>";
	...
	jQuery("#material_inventory_picklist_picking_document_btn").click(function(){
		...
		{
			var row_data = jQuery("#material_inventory_picklist_list_table").getRowData(id);
			window.open("<?php echo site_url('material/inventory_picklist_pakan/picking_document');?>?id=" + row_data.id + "&c_orderout_id=" + row_data.c_orderout_id);
		}
		else
			jquery_show_message("Please select the row data !", null, "ui-icon-alert");
	});
	...
	jQuery("#material_inventory_picklist_tally_sheet_btn").click(function(){
		...
		if (id)
		{
			...
			window.open("<?php echo site_url('material/inventory_picklist_pakan/tally_sheet');?>?" + params);
		}
		else
			jquery_show_message("Please select the row data !", null, "ui-icon-alert");
	});
...
function material_inventory_picklist_list_load_table(table_id){
	jQuery('#' + table_id).jqGrid({
		...
		url: "<?php echo site_url('material/inventory_picklist_pakan/get_list_json');?>", 
		postData : material_inventory_picklist_list_get_param(),
		colNames: [
			'Id', 
			'Order Out Id', 
		...
		colModel: [
			...
			jqgrid_column_time(table_id, {name:'schedule_time', index:'ipl.schedule_time'}),
			{name:'supervisor', index:'ipl.supervisor', width:120, searchoptions:{sopt:jqgird_search_string_operators, clearSearch:false}},
			{name:'schedule_phase', index:'ipl.schedule_phase', width:90, align:'center', 
			 stype:'select', searchoptions:{sopt:jqgird_search_string_operators, clearSearch:false, value:<?php echo json_encode($inventory_picklist_schedule_phase);?>},
			},
...
function material_inventory_picklist_list_detail(id){
	jquery_dialog_form_open('material_inventory_picklist_detail_container', "<?php echo site_url('material/inventory_picklist_pakan/detail');?>", {
		id : id
	}, null,
	{
		title : "Pemilihan", 
