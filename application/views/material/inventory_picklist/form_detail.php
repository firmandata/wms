...
<?php
echo form_open('material/inventory_picklist/insert_detail',
	array(
...
echo form_input(
	array(
		...
		'style'		=> "width:180px"
									...
									<tr><th><label for="material_inventory_picklist_form_detail_grid_code">Grid</label></th>
										<td>
<?php 
echo form_input(
	array(
...
function material_inventory_picklist_detail_ref_list_load_table(table_id){
	jQuery('#' + table_id).jqGrid({
		...
		url: "<?php echo site_url('material/inventory_picklist/get_list_form_ref_json');?>", 
		colNames: [
			'Id', 
			'Product Id', 
			'Product Code', 
		...
		colModel: [
			...
			{name:'c_orderout_origin', index:'oo.origin', width:100,
...
function material_inventory_picklist_detail_inventory_list_load_table(table_id){
	jQuery('#' + table_id).jqGrid({
		...
		width: jqgrid_window_fixed_width(table_id),
		url: "<?php echo site_url('material/inventory_picklist/get_form_inventory_list_json');?>", 
		postData : material_inventory_picklist_detail_ref_list_get_param(),
		colNames: [
			'Product Id', 
			'Grid Id',
...
function material_inventory_picklist_detail_list_load_table(table_id){
	jQuery('#' + table_id).jqGrid({
		...
		width: jqgrid_window_fixed_width(table_id),
		url: "<?php echo site_url('material/inventory_picklist/get_list_form_json');?>", 
		postData : {
			id : <?php echo $id;?>
		},
		colNames: [
...
function material_inventory_picklist_detail_list_remove(table_id, id){
	jquery_show_confirm("Are your sure ?", function(){
		...
		jQuery.ajax({
			url: "<?php echo site_url('material/inventory_picklist/delete_detail');?>",
			type: "POST",
			dataType: "json",
			data : {
...
function material_inventory_picklist_detail_show_total(){
	...
	jQuery.ajax({
		url: "<?php echo site_url('material/inventory_picklist/get_detail_counter');?>",
		data : {
			c_orderoutdetail_id		: ref_row_data.id,
			m_inventory_picklist_id	: <?php echo $id;?>
		},
		...
		success: function(data, textStatus, jqXHR){
			...
			else
			{
				...
				var total_label = "Free : " + (c_order_out.quantity_box - c_order_out.quantity_box_used) + "/" + (c_order_out.quantity - c_order_out.quantity_used);
				var counter_label = "Allocated : " + m_inventory_picklistdetail.quantity_box + "/" + m_inventory_picklistdetail.quantity;
...
function material_inventory_picklist_detail_process_manual(){
	...
	if (records.length > 0)
	{
		...
		jQuery.ajax({
			url: "<?php echo site_url('material/inventory_picklist/insert_detail_manual');?>",
			type: "POST",
			dataType: "json",
			async : false,
			data : {
...
function material_inventory_picklist_detail_process_manual_show_total(){
	...
	jQuery("#material_inventory_picklist_detail_form_tab_enroll_manual_table_selected_total").text("Selected Quantity : " + total_quantity_box + "/" + total_quantity);
}
</script>