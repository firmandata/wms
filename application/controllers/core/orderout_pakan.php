...
class Orderout_pakan extends MY_Controller 
{
	...
	{
		if (!is_authorized('core/orderout_pakan', 'index')) 
			access_denied();
		...
		$content = array(
			'title'		=> "SPK",
			'content' 	=> $this->load->view('core/orderout_pakan/index', $data, TRUE)
		);
	...
	public function get_list_json()
	{
		...
		$this->db
			...
			->select("oo.origin, oo.marketing_unit, oo.external_no, oo.no_surat_jalan, oo.approval_by")
		...
		$this->db->where("oo.orderout_type", 1);
	...
	public function form()
	{
		...
		$this->load->view('core/orderout_pakan/form', $data);
	}
	...
	public function form_upload()
	{
		...
		$this->load->view('core/orderout_pakan/form_upload', $data);
	}
	...
	public function detail()
	{
		...
		$this->load->view('core/orderout_pakan/detail', $data);
	}
	...
	public function get_product_autocomplete_list_json()
	{
		...
		$this->db
			...
			->select_concat(array("name", "' ('", "code", "')'"), 'label')
			->select("netto, pack")
			->select("type, casing, uom")
			->from('m_products')
			->where_not_in('type', array('BENUR/BIBIT', 'UDANG'));
			...
			try
			{
				...
				foreach ($excel_sheets as $excel_sheet_name=>$excel_sheet)
				{
					try
					{
						...
						$c_order->external_no = $this->excel->read_value($excel_sheet, 4, 6);
						$c_order->approval_by = $this->excel->read_value($excel_sheet, 5, 6);
						...
						for ($c_orderoutdetail_count = 10; $c_orderoutdetail_count < count($excel_sheet); $c_orderoutdetail_count++)
						{
							...
							$c_orderoutdetail->quantity = $this->excel->read_value($excel_sheet, $c_orderoutdetail_count, 5);
							$c_orderoutdetail->notes = $this->excel->read_value($excel_sheet, $c_orderoutdetail_count, 7);
				...
				if (count($c_orders) > 0)
				{
					...
					try
					{
						foreach ($c_orders as $c_order)
						{
							...
							$data_header->approval_by = $c_order->approval_by;
	...
	public function detail_printout()
	{
		...
		$html = $this->load->view('core/orderout_pakan/detail_printout', $data, TRUE);
