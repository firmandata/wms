...
class Inventory_picklist_pakan extends MY_Controller 
{
	...
	{
		if (!is_authorized('material/inventory_picklist_pakan', 'index')) 
			access_denied();
		...
		$content = array(
			'title'		=> "Pemberian",
			'content' 	=> $this->load->view('material/inventory_picklist_pakan/index', $data, TRUE)
		);
	...
	public function get_list_json()
	{
		...
		$this->db
			...
			->select("ipl.m_grid_id, gri.code m_grid_code")
			...
			->join('m_grids gri', "gri.id = ipl.m_grid_id", 'left')
		...
		$this->db->where("ipl.picklist_orderout_type", 1);
	...
	public function form()
	{
		...
		$this->load->view('material/inventory_picklist_pakan/form', $data);
	}
	...
	public function form_detail()
	{
		...
		$this->load->view('material/inventory_picklist_pakan/form_detail', $data);
	}
	...
	public function get_list_form_json()
	{
		...
		$this->db
			...
			->select("ipld.received_date, ipld.price_buy")
			...
			->group_by(
				array(
					...
					'ipld.received_date', 'ipld.price_buy',
	...
	public function get_form_inventory_list_json()
	{
		...
		$inventory_sql = 
			...
			."			, inv.received_date "
			."			, inv.quantity_per_box, inv.price_buy "
		...
		$this->db
			...
			->select("inv.barcode, inv.pallet, inv.carton_no, inv.packed_date, inv.expired_date, inv.condition, inv.lot_no, inv.quantity_per_box, inv.price_buy")
	...
	public function detail()
	{
		...
		$this->load->view('material/inventory_picklist_pakan/detail', $data);
	}
	...
	public function insert()
	{
		...
		$data = new stdClass();
		$data->code = generate_code_number("SPKPL". date('ymd-'), NULL, 3);
		$data->picklist_date = $this->input->post('picklist_date');
		$data->picklist_orderout_type = 1;
		$data->supervisor = $this->input->post('supervisor');
		$data->schedule_time = $this->input->post('schedule_time');
		$data->schedule_phase = $this->input->post('schedule_phase');
		$data->m_grid_id = $this->input->post('m_grid_id');
		...
		parent::_execute('lib_inventory_out', 'picklist_add', 
			...
			array(
				...
				array('field' => 'm_grid_id', 'label' => 'Location', 'rules' => 'required')
			)
		);
	}
	...
	protected function _insert_detail_manual()
	{
		...
		if (!empty($records) && is_array($records))
		{
			foreach ($records as $record_idx=>$record)
			{
				...
				$data->price_buy = $record['price_buy'];
