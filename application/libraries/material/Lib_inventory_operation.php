...
class Lib_inventory_operation extends Lib_general
{
	...
	public function assemblysource_add($data, $created_by = NULL)
	{
		...
		if (property_exists($data, 'm_inventory_id'))
		{
			...
			$m_inventory_assemblysource_relations['c_businesspartner'] = $m_inventory->c_businesspartner->get();
			unset($data->m_inventory_id);
	...
	{
		$fields = array(
			'm_grid_id', 'm_product_id', 'c_project_id', 'c_businesspartner_id', 'pallet', 'barcode', 'carton_no', 'lot_no', 'volume_length', 'volume_width', 'volume_height'
		);
	...
	public function assemblytarget_add($data, $created_by = NULL)
	{
		...
		if (property_exists($data, 'c_businesspartner_id'))
		{
			...
			$assemblytarget_relations['c_businesspartner'] = $c_businesspartner;
	...
	public function invoice_add($data, $created_by = NULL)
	{
		$invoice_relations = array();
		if (property_exists($data, 'c_businesspartner_id'))
		{
			$invoice_relations['c_businesspartner'] = new C_businesspartner($data->c_businesspartner_id);
			unset($data->c_businesspartner_id);
		}
		
		$data->created_by = $created_by;
		
		$m_inventory_invoice = new M_inventory_invoice();
		$this->set_model_fields_values($m_inventory_invoice, $data);
		$m_inventory_invoice_saved = $m_inventory_invoice->save($invoice_relations);
		if (!$m_inventory_invoice_saved)
			throw new Exception($m_inventory_invoice->error->string);
		
		// -- Generate invoicedetails --
		$this->invoicedetails_generate($m_inventory_invoice->id, $created_by);
		
		return $m_inventory_invoice->id;
	}
	
	public function invoice_update($m_inventory_invoice_id, $data, $updated_by = NULL)
	{
		$invoice_relations = array();
		if (property_exists($data, 'c_businesspartner_id'))
		{
		...
		$m_inventory_invoice = new M_inventory_invoice($m_inventory_invoice_id);
		$this->set_model_fields_values($m_inventory_invoice, $data);
		$m_inventory_invoice_saved = $m_inventory_invoice->save($invoice_relations);
		if (!$m_inventory_invoice_saved)
			throw new Exception($m_inventory_invoice->error->string);
		
		// -- Regenerate invoicedetails --
		$this->invoicedetails_generate($m_inventory_invoice->id, $updated_by);
		
		return $m_inventory_invoice_id;
	}
	
	protected function invoicedetails_generate($m_inventory_invoice_id, $generate_by = NULL)
	{
		$m_inventory_invoice = new M_inventory_invoice($m_inventory_invoice_id);
		
		$m_inventory_invoicedetail_in = NULL;
		$m_inventory_invoicedetail_out = NULL;
		$m_inventory_invoicedetail_storage = NULL;
		
		$m_inventory_invoicedetail_updated_ids = array();
		$m_inventory_invoicedetails = $m_inventory_invoice->m_inventory_invoicedetail->get();
		
		// -- Populate Inventory Invoice Detail --
		foreach ($m_inventory_invoicedetails as $m_inventory_invoicedetail)
		{
			if ($m_inventory_invoicedetail->invoice_type == 'HANDLING IN')
				$m_inventory_invoicedetail_in = $m_inventory_invoicedetail;
			if ($m_inventory_invoicedetail->invoice_type == 'HANDLING OUT')
				$m_inventory_invoicedetail_out = $m_inventory_invoicedetail;
			if ($m_inventory_invoicedetail->invoice_type == 'STORAGE')
				$m_inventory_invoicedetail_storage = $m_inventory_invoicedetail;
		}
		
		if ($m_inventory_invoice->invoice_handling_in)
		{
			if ($m_inventory_invoicedetail_in === NULL)
				$this->invoicedetails_generate_handling($m_inventory_invoice->id, 'HANDLING IN', NULL, $generate_by);
			else
			{
				$m_inventory_invoicedetail_updated_ids[] = $m_inventory_invoicedetail_in->id;
				$this->invoicedetails_generate_handling($m_inventory_invoice->id, 'HANDLING IN', $m_inventory_invoicedetail_in->id, $generate_by);
			}
		}
		if ($m_inventory_invoice->invoice_handling_out)
		{
			if ($m_inventory_invoicedetail_out === NULL)
				$this->invoicedetails_generate_handling($m_inventory_invoice->id, 'HANDLING OUT', NULL, $generate_by);
			else
			{
				$m_inventory_invoicedetail_updated_ids[] = $m_inventory_invoicedetail_out->id;
				$this->invoicedetails_generate_handling($m_inventory_invoice->id, 'HANDLING OUT', $m_inventory_invoicedetail_out->id, $generate_by);
			}
		}
		if ($m_inventory_invoice->invoice_handling_storage)
		{
			if ($m_inventory_invoicedetail_storage === NULL)
				$this->invoicedetails_generate_handling($m_inventory_invoice->id, 'STORAGE', NULL, $generate_by);
			else
			{
				$m_inventory_invoicedetail_updated_ids[] = $m_inventory_invoicedetail_storage->id;
				$this->invoicedetails_generate_handling($m_inventory_invoice->id, 'STORAGE', $m_inventory_invoicedetail_storage->id, $generate_by);
			}
		}
		
		// -- Remove Unused Inventory Invoice Detail --
		foreach ($m_inventory_invoicedetails as $m_inventory_invoicedetail)
		{
			if (!in_array($m_inventory_invoicedetail->id, $m_inventory_invoicedetail_updated_ids))
			{
				if (!$m_inventory_invoicedetail->delete())
					throw new Exception($m_inventory_invoicedetail->error->string);
			}
		}
	}
	
	protected function invoicedetails_generate_handling($m_inventory_invoice_id, $invoice_type, $m_inventory_invoicedetail_id = NULL, $generate_by = NULL)
	{
		$m_inventory_invoice = new M_inventory_invoice($m_inventory_invoice_id);
		$c_businesspartner = $m_inventory_invoice->c_businesspartner->get();
		
		$period_from = date($this->CI->config->item('server_date_format'), strtotime($m_inventory_invoice->period_from)) . ' 00:00:00';
		$period_from_display = date('d M Y', strtotime($m_inventory_invoice->period_from));
		$period_to = date($this->CI->config->item('server_date_format'), strtotime($m_inventory_invoice->period_to)) . ' 23:59:59';
		$period_to_display = date('d M Y', strtotime($m_inventory_invoice->period_to));
		
		$parts_num = 0;
		$weight = 0;
		$amount = 0;
		
		if ($invoice_type == 'HANDLING IN')
		{
			$quantities = $this->get_invoice_handling_in_quantities($c_businesspartner->id, $m_inventory_invoice->invoice_calculate, $period_from, $period_to);
			$parts_num = $quantities['parts_num'];
			$weights = $quantities['weights'];
			foreach ($weights as $weight_uom=>$weight_quantity)
			{
				if (strtoupper($weight_uom) == 'KG')
					$weight += $weight_quantity;
				elseif (strtoupper($weight_uom) == 'GR')
					$weight += ($weight_quantity * 1000);
			}
		...
		elseif ($invoice_type == 'HANDLING OUT')
		{
			$quantities = $this->get_invoice_handling_out_quantities($c_businesspartner->id, $m_inventory_invoice->invoice_calculate, $period_from, $period_to);
			$parts_num = $quantities['parts_num'];
			$weights = $quantities['weights'];
			foreach ($weights as $weight_uom=>$weight_quantity)
			{
		...
		elseif ($invoice_type == 'STORAGE')
		{
			$quantities = $this->get_invoice_handling_storage_quantities($c_businesspartner->id, $m_inventory_invoice->invoice_calculate, $period_from, $period_to);
			$parts_num = $quantities['parts_num'];
			$weights = $quantities['weights'];
			foreach ($weights as $weight_uom=>$weight_quantity)
			{
		...
		$data->description = $data->invoice_type." CHARGES PERIODE ". strtoupper($period_from_display)." - ".strtoupper($period_to_display);
		$data->parts_num = $parts_num;
		$data->amount = $amount;
		$data->weight = $weight;
		if ($m_inventory_invoicedetail_id === NULL)
			$data->created_by = $generate_by;
		else
			$data->updated_by = $generate_by;
		
		$m_inventory_invoicedetail_relations = array(
			'm_inventory_invoice'	=> $m_inventory_invoice
		);
		
		if ($m_inventory_invoicedetail_id === NULL)
			$m_inventory_invoicedetail = new M_inventory_invoicedetail();
		else
			$m_inventory_invoicedetail = new M_inventory_invoicedetail($m_inventory_invoicedetail_id);
		$this->set_model_fields_values($m_inventory_invoicedetail, $data);
		$m_inventory_invoicedetail_saved = $m_inventory_invoicedetail->save($m_inventory_invoicedetail_relations);
		if (!$m_inventory_invoicedetail_saved)
			throw new Exception($m_inventory_invoicedetail->error->string);
	}
	
	protected function get_invoice_handling_in_quantities($c_businesspartner_id, $invoice_calculate, $period_from, $period_to)
	{
		$weights = array();
		$parts_num = 0;
		$volume_min = 0;
		if (in_array($invoice_calculate, array('BOX', 'WEIGHT', 'PALLET')))
		{
			...
			elseif ($invoice_calculate == 'PALLET')
				$this->CI->db
					->select_if_null("COUNT(DISTINCT iid.pallet)", 0, 'pallet');
			$this->CI->db
				->select_if_null("SUM(iid.quantity)", 0, 'quantity');
			$this->CI->db
				->select('pro.uom')
				->from('m_inventory_inbounddetails iid')
				->join('m_products pro', "pro.id = iid.m_product_id", 'left')
				->where('iid.c_businesspartner_id', $c_businesspartner_id)
				->where('iid.created >=', $period_from)
	...
	protected function get_invoice_handling_out_quantities($c_businesspartner_id, $invoice_calculate, $period_from, $period_to)
	{
		$weights = array();
		$parts_num = 0;
		$volume_min = 0;
		if (in_array($invoice_calculate, array('BOX', 'WEIGHT', 'PALLET')))
		{
			...
			elseif ($invoice_calculate == 'PALLET')
				$this->CI->db
					->select_if_null("COUNT(DISTINCT ipld.pallet)", 0, 'pallet');
			$this->CI->db
			...
			$this->CI->db
				...
				->where('ipld.c_businesspartner_id', $c_businesspartner_id)
				->where('ispd.created >=', $period_from)
	...
	protected function get_invoice_handling_storage_quantities($c_businesspartner_id, $invoice_calculate, $period_from, $period_to)
	{
		...
		if (in_array($invoice_calculate, array('BOX', 'WEIGHT', 'PALLET')))
		{
			...
			elseif ($invoice_calculate == 'PALLET')
				$this->CI->db
					->select_if_null("COUNT(DISTINCT invl.pallet)", 0, 'pallet');
			$this->CI->db
				->select_if_null("SUM(invl.quantity)", 0, 'quantity')
				->select('pro.uom')
				->from('m_inventorylogs invl')
				->join('m_products pro', "pro.id = invl.m_product_id", 'left')
				->where('invl.c_businesspartner_id', $c_businesspartner_id)
				->where('invl.created <', $period_from)
				->where('invl.quantity <>', 0)
	...
	public function invoice_remove($m_inventory_invoice_id, $removed_by = NULL)
	{
		$m_inventory_invoice = new M_inventory_invoice($m_inventory_invoice_id);
		
		// -- Remove Inventory Invoice Detail --
		foreach ($m_inventory_invoice->m_inventory_invoicedetail->get() as $m_inventory_invoicedetail)
		{
			if (!$m_inventory_invoicedetail->delete())
				throw new Exception($m_inventory_invoicedetail->error->string);
		}
		
		// -- Remove Inventory Invoice --
		if (!$m_inventory_invoice->delete())
			throw new Exception($m_inventory_invoice->error->string);
